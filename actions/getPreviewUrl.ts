...
export default async function getPreviewUrl(trackId: string) {
    const audioPreviewRegex = /"audioPreview":\{"url":"(https:\/\/p\.scdn\.co\/mp3-preview\/[a-zA-Z0-9-]+)"\}/;
    const res = await fetch(`https://open.spotify.com/embed/track/${trackId}?utm_source=generator`);
